...
def generateApacheLogDf(filepath):
    mycols = ["IP", "Identity", "Username", "Date", "Date 2", "Request Method", "Response Code", "Bytes Out", "Referer",
              "User Agent"]
...
def generateBruteForceLogs(df):
    ...
    for index, entry in df.iterrows():
        ...
        if "POST" in entry[3]:
            if isAnomalyFound == False:
                ...
                # print "IP:"+entry[0]
                isAnomalyFound = True
                bigList.append(pd.DataFrame(
                    columns=['1-IP', '2-Username', '3-Date', '4-Request Method', '5-Response Code', '6-Bytes Out',
                ...
                for index in range(len(bigList)):
                    ...
                    if bigList[index].loc[len(bigList[index]) - 1, '1-IP'] == entry[0]:  # If IP Matches
                        ...
                        previousLogTime = datetime.datetime.strptime(
                            bigList[index].loc[len(bigList[index]) - 1, '3-Date'], "%d/%b/%Y:%H:%M:%S")
    ...
    for IP in attackerIPs:
        for index in range(len(df)):
            if IP == df['1-IP'].iloc[index] and "POST" in df['4-Request Method'].iloc[index] and loginpage in \
                    df['4-Request Method'].iloc[index]:  # If IP match and it is a POST request to a login page
                nextIndex = index + 1
                if IP == df['1-IP'].iloc[nextIndex] and "GET" in df['4-Request Method'].iloc[
                    nextIndex] and loginpage not in df['4-Request Method'].iloc[
                    nextIndex]:  # If next entry's IP matches attacker IP, and is a GET request to a homepage
    ...
    for IP in attackerIPs:
        for index, entry in df.iterrows():
            if IP == entry[0] and homepage in entry[3] and "GET" in entry[
